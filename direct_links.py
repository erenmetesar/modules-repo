...
class DirectGen:
    ...
    async def directxxx(request):
        """ direct links generator """
        await request.edit("<code>Processing...</code>")
        textx = await request.get_reply_message()
        message = utils.get_arg(request)
        if message:
            pass
        elif textx:
            message = textx.text
        else:
            await request.edit("<code>Usage: .direct <url></code>")
            return
        reply = ''
        links = re.findall(r'\bhttps?://.*\.\S+', message)
        if not links:
            reply = "<code>No links found!</code>"
